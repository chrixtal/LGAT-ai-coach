...
# ============================
# Backend Function Helpers (Base44 sync)
# ============================

def call_backend_function(func_name, payload):
    """呼叫 Base44 backend function，同步資料"""
    ...
    if not BASE44_SERVICE_TOKEN or not BASE44_APP_ID:
        print(f"[Backend] 缺少憑證，無法呼叫 {func_name}")
        return None
    
    try:
        resp = requests.post(
            f'{BASE44_API_URL}/apps/{BASE44_APP_ID}/functions/{func_name}',
            headers={
        ...
        if resp.status_code == 200:
            print(f"[Backend] {func_name} 成功")
            return resp.json()
        else:
            print(f"[Backend] {func_name} 失敗 ({resp.status_code})")
            return None
...
def sync_user_to_base44(line_user_id, profile):
    ...
    try:
        payload = {
            ...
            'coach_style': profile.get('coach_style', 'exploratory'),
            'quote_freq': profile.get('quote_freq', 'sometimes'),
            'total_messages': profile.get('total_messages', 1),
        }
...
def detect_and_save_goal_or_event(line_user_id, display_name, user_text, ai_response):
    """從對話中偵測關鍵詞，自動儲存目標/事件到 Base44"""
    try:
        # 關鍵詞偵測
        goal_keywords = ['目標', '想要', '計畫', '達成', '完成', '學習', '改變', '目的']
        event_keywords = ['習慣', '待辦', '每天', '每週', '里程碑', '打卡', '記錄']
        ...
        if any(kw in combined for kw in event_keywords):
            recurrence = 'daily' if '每天' in combined else ('weekly' if '每週' in combined else 'none')
            payload = {
                'entity_type': 'event',
                'line_user_id': line_user_id,
                'display_name': display_name,
        ...
        print(f"[Base44] detect_and_save 異常: {e}")
...
def handle_message(event):
    ...
    def process_and_push():
        ...
        detect_and_save_goal_or_event(user_id, display_name, user_text, ai_response)
    ...
    # 同步用戶資料到 Base44
    threading.Thread(target=lambda: sync_user_to_backend(user_id, profile), daemon=True).start()
