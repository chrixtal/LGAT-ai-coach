...
if not all([LINE_CHANNEL_ACCESS_TOKEN, LINE_CHANNEL_SECRET, DIFY_API_KEY]):
    print("❌ 錯誤: 缺少必要的環境變數設定（LINE_CHANNEL_ACCESS_TOKEN / LINE_CHANNEL_SECRET / DIFY_API_KEY）")

line_bot_api = LineBotApi(LINE_CHANNEL_ACCESS_TOKEN)
handler = WebhookHandler(LINE_CHANNEL_SECRET)

# --- SQLite 初始化（暫存，後期全遷到 Base44） ---
...
def save_goal_or_event_to_base44(entity_type, line_user_id, display_name='', **fields):
    """呼叫 saveGoalOrEvent backend function，儲存目標或事件"""
    try:
        ...
        resp = requests.post(url, json=payload, timeout=10)
        if resp.status_code == 200:
            print(f"✅ [Base44] 儲存 {entity_type} 成功: {line_user_id}")
            return True
        else:
            print(f"❌ [Base44] 儲存 {entity_type} 失敗 ({resp.status_code}): {resp.text}")
            return False
    except Exception as e:
        print(f"❌ [Base44] 儲存 {entity_type} 異常: {e}")
        return False
...
def detect_and_save_goals_events(line_user_id, display_name, text):
    """
    偵測訊息中的目標/事件關鍵詞，自動儲存到 Base44
    """
    # 目標關鍵詞：目標、想要、想達成、計畫、要完成
    goal_keywords = ['目標', '想要', '想達成', '計畫', '要完成', '新年願望', '夢想', '想學']
    
    # 習慣/事件關鍵詞：習慣、待辦、要做、打卡、紀錄
    event_keywords = ['習慣', '待辦', '要做', '打卡', '紀錄', '每天', '每週']
    ...
    if is_goal:
        # 簡單抽取標題（取前 30 個字）
        title = re.sub(r'[（）、，。！？；：]', '', text)[:30]
        save_goal_or_event_to_base44('goal', line_user_id, display_name,
                                      title=title, description=text, type='short')
        print(f"[自動儲存] 目標: {title}")
    
    if is_event:
        title = re.sub(r'[（）、，。！？；：]', '', text)[:30]
        save_goal_or_event_to_base44('event', line_user_id, display_name,
                                      title=title, type='todo', note=text)
        print(f"[自動儲存] 事件: {title}")
...
def handle_message(event):
    ...
    def process_and_push():
        # 同步用戶資料到 Base44（背景執行）
        threading.Thread(
            target=lambda: sync_user_to_base44(
                user_id,
        ...
        # 自動偵測目標/事件（背景執行）
        threading.Thread(
            target=lambda: detect_and_save_goals_events(user_id, profile.get('display_name', ''), user_text),
            daemon=True
        ).start()
