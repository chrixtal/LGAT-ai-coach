...
def init_db():
    ...
    c = conn.cursor()
    c.execute('''CREATE TABLE IF NOT EXISTS user_conversations (line_user_id TEXT PRIMARY KEY, conversation_id TEXT, created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP, updated_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP)''')
    c.execute('''CREATE TABLE IF NOT EXISTS user_profiles (line_user_id TEXT PRIMARY KEY, display_name TEXT, coach_tone TEXT DEFAULT 'balanced', coach_style TEXT DEFAULT 'exploratory', quote_freq TEXT DEFAULT 'sometimes', onboarding_done INTEGER DEFAULT 0, onboarding_step INTEGER DEFAULT 0, total_messages INTEGER DEFAULT 0, created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP, updated_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP)''')
    conn.commit()
    conn.close()
...
def save_conversation_id(line_user_id, conversation_id):
    conn = sqlite3.connect(DB_PATH)
    c = conn.cursor()
    c.execute('INSERT INTO user_conversations (line_user_id, conversation_id, updated_at) VALUES (?, ?, CURRENT_TIMESTAMP) ON CONFLICT(line_user_id) DO UPDATE SET conversation_id = excluded.conversation_id, updated_at = CURRENT_TIMESTAMP', (line_user_id, conversation_id))
    conn.commit()
    conn.close()
...
def send_loading_animation(user_id, seconds=20):
    try:
        resp = requests.post('https://api.line.me/v2/bot/chat/loading/start', headers={'Authorization': f'Bearer {LINE_CHANNEL_ACCESS_TOKEN}', 'Content-Type': 'application/json'}, json={'chatId': user_id, 'loadingSeconds': seconds}, timeout=5)
...
def sync_user_to_base44(user_id, display_name, coach_tone, coach_style, quote_freq, total_messages):
    """同步用戶資料到 Base44"""
    try:
        data = {'line_user_id': user_id, 'display_name': display_name, 'coach_tone': coach_tone, 'coach_style': coach_style, 'quote_freq': quote_freq, 'total_messages': total_messages}
        resp = requests.post(f'{BASE44_API_URL}/functions/syncUser', json=data, timeout=5)
        print(f"[Base44 Sync] {resp.status_code} | {user_id}")
    except Exception as e:
        print(f"[Base44 Sync] 錯誤: {e}")

def detect_and_save_goal_or_event(user_id, display_name, user_text):
    """偵測用戶訊息中的關鍵詞，自動儲存目標或事件"""
    try:
        goal_keywords = ['目標', '想要', '計畫', '期望', '想做']
        event_keywords = ['習慣', '提醒', '待辦', '要做']
        
        if any(kw in user_text for kw in goal_keywords):
            title = user_text.split('：')[1].strip()[:30] if '：' in user_text else user_text[:20].strip()
            resp = requests.post(f'{BASE44_API_URL}/functions/saveGoalOrEvent', json={'entity_type': 'goal', 'line_user_id': user_id, 'display_name': display_name, 'title': title, 'description': user_text, 'type': 'short'}, timeout=5)
            print(f"[Goal Save] {resp.status_code}")
        
        if any(kw in user_text for kw in event_keywords):
            title = user_text.split('：')[1].strip()[:30] if '：' in user_text else user_text[:20].strip()
            event_type = 'habit' if any(kw in user_text for kw in ['習慣']) else 'todo'
            resp = requests.post(f'{BASE44_API_URL}/functions/saveGoalOrEvent', json={'entity_type': 'event', 'line_user_id': user_id, 'display_name': display_name, 'title': title, 'type': event_type}, timeout=5)
            print(f"[Event Save] {resp.status_code}")
    except Exception as e:
        print(f"[detect_goal_event] 錯誤: {e}")
...
def _tone_question():
    emojis = ['1️⃣', '2️⃣', '3️⃣']
    text = "❷ 你喜歡什麼樣的教練語氣？\n\n請輸入數字：\n"
    for i, v in enumerate(TONE_OPTIONS.values()):
        text += f"{emojis[i]} {v['label']}\n"
    return text

def _style_question():
    emojis = ['1️⃣', '2️⃣']
    text = "❸ 你習慣哪種溝通方式？\n\n請輸入數字：\n"
    for i, v in enumerate(STYLE_OPTIONS.values()):
        text += f"{emojis[i]} {v['label']}\n"
    return text
...
def _quote_question():
    emojis = ['1️⃣', '2️⃣', '3️⃣']
    text = "❹ 你喜歡我引用名言或理論嗎？\n\n請輸入數字：\n"
    for i, v in enumerate(QUOTE_OPTIONS.values()):
        text += f"{emojis[i]} {v['label']}\n"
    return text
...
def handle_message(event):
    ...
    def process_and_push():
        try:
            ...
            msg_count = current_profile.get('total_messages', 0) + 1
            ...
            if not display_name:
                ...
                if display_name:
                    save_profile(user_id, display_name=display_name)
            
            # 同步用戶資料到 Base44
            sync_user_to_base44(user_id, display_name, current_profile.get('coach_tone', 'balanced'), current_profile.get('coach_style', 'exploratory'), current_profile.get('quote_freq', 'sometimes'), msg_count)
            
            # 偵測目標/事件
            detect_and_save_goal_or_event(user_id, display_name, user_text)
