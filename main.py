...
TONE_OPTIONS = _parse_options(os.environ.get('COACH_TONE_OPTIONS',
    'strict:嚴格督促型（推你一把，不留情面）:嚴格督促|gentle:溫柔支持型（像朋友一樣陪伴你）:溫柔支持|balanced:平衡理性型（視情況調整）:平衡理性'))
STYLE_OPTIONS = _parse_options(os.environ.get('COACH_STYLE_OPTIONS',
    'direct:直接說重點（我要答案，不要繞彎子）:直接說重點|exploratory:循循善誘（陪我慢慢想清楚）:循循善誘、引導探索'))
QUOTE_OPTIONS = _parse_options(os.environ.get('COACH_QUOTE_OPTIONS',
...
def sync_user_to_base44(user_id, profile):
    try:
        resp = requests.post(f'{BASE44_API_URL}/functions/syncUser',
            json={'line_user_id': user_id, 'display_name': profile.get('display_name', ''), 'coach_tone': profile.get('coach_tone'), 'coach_style': profile.get('coach_style'), 'quote_freq': profile.get('quote_freq'), 'total_messages': profile.get('total_messages', 0)},
            timeout=5)
...
def detect_and_save_goal_or_event(user_id, display_name, text):
    try:
        ...
        if entity_type:
            ...
            title = title.replace('\n', ' ').strip()
            resp = requests.post(f'{BASE44_API_URL}/functions/saveGoalOrEvent',
                json={'entity_type': entity_type, 'line_user_id': user_id, 'display_name': display_name, 'title': title},
                timeout=5)
            if resp.status_code == 200:
                print(f"[save {entity_type}] OK {user_id}")
    except Exception as e:
        print(f"[save goal/event] {e}")
...
def _tone_question():
    emojis = ['1️⃣', '2️⃣', '3️⃣']
    opts = '\n'.join(f"{emojis[i]} {v['label']}" for i, v in enumerate(TONE_OPTIONS.values()))
    return f"❷ 你喜歡什麼樣的教練語氣？\n\n請輸入數字：\n{opts}"

def _style_question():
    emojis = ['1️⃣', '2️⃣']
    opts = '\n'.join(f"{emojis[i]} {v['label']}" for i, v in enumerate(STYLE_OPTIONS.values()))
    return f"❸ 你習慣哪種溝通方式？\n\n請輸入數字：\n{opts}"

def _quote_question():
    emojis = ['1️⃣', '2️⃣', '3️⃣']
    opts = '\n'.join(f"{emojis[i]} {v['label']}" for i, v in enumerate(QUOTE_OPTIONS.values()))
...
def handle_onboarding(line_user_id, text, profile):
    ...
    if step == 2:
        ...
        if not opt:
            return f"請輸入 1-{len(TONE_OPTIONS)} 其中一個 😊\n\n{_tone_question()}"
    ...
    if step == 3:
        ...
        if not opt:
            return f"請輸入 1-{len(STYLE_OPTIONS)} 其中一個 😊\n\n{_style_question()}"
    ...
    if step == 4:
        ...
        if not opt:
            return f"請輸入 1-{len(QUOTE_OPTIONS)} 其中一個 😊\n\n{_quote_question()}"
