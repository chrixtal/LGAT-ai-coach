...
BASE44_API_URL = f'{BASE44_APP_URL}/functions'
...
def init_db():
    ...
    c.execute('''CREATE TABLE IF NOT EXISTS user_profiles
        (line_user_id TEXT PRIMARY KEY, display_name TEXT,
         ...
         quote_freq TEXT DEFAULT 'sometimes', total_messages INTEGER DEFAULT 0,
...
def sync_user_to_base44(user_id, profile):
    """呼叫 syncUser function 同步用戶到 Base44"""
    try:
        msg_count = profile.get('total_messages', 0)
        resp = requests.post(
...
def handle_message(event):
    ...
    def process_and_push():
        ...
        if any(kw in text_lower for kw in ['目標', '想要', '計畫', '準備']):
            save_goal_to_base44(user_id, current_profile.get('display_name', ''), user_text[:50], 'short')
        elif any(kw in text_lower for kw in ['完成', '打卡', '做', '習慣', '待辦']):
            save_event_to_base44(user_id, current_profile.get('display_name', ''), user_text[:50], 'todo')
