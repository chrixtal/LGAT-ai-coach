...
def call_base44_api(endpoint, data, timeout=10):
    """呼叫 Base44 backend function"""
    try:
        ...
        if response.status_code == 200:
            ...
            print(f"[Base44] {endpoint} 失敗: {response.status_code}")
            return None
    except Exception as e:
        print(f"[Base44] {endpoint} 錯誤: {e}")
        return None

def sync_user_to_base44(user_id, profile):
    """同步用戶資料到 Base44"""
    threading.Thread(target=lambda: call_base44_api('syncUser', {
...
def detect_and_save_goals_events(user_id, text, profile):
    """偵測訊息中的目標/事件關鍵詞，自動存入 Base44"""
    goal_keywords = ['目標', '想要', '計畫', '希望', '夢想', '想達成', '應該', '需要']
    event_keywords = ['待辦', '事項', '習慣', '打卡', '完成', '做', '跑步', '健身', '閱讀', '冥想']

    text_lower = text.lower()
    for keyword in goal_keywords:
        if keyword in text:
            idx = text.find(keyword)
            snippet = text[max(0, idx-20):min(len(text), idx+50)].strip()
            call_base44_api('saveGoalOrEvent', {
                'entity_type': 'goal',
                'line_user_id': user_id,
                'display_name': profile.get('display_name', ''),
                'title': snippet or text[:30],
                'type': 'short',
            })
            break
    ...
    for keyword in event_keywords:
        if keyword in text:
            idx = text.find(keyword)
            snippet = text[max(0, idx-10):min(len(text), idx+40)].strip()
            call_base44_api('saveGoalOrEvent', {
                'entity_type': 'event',
                'line_user_id': user_id,
                'display_name': profile.get('display_name', ''),
