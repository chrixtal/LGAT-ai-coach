...
# Backend function base URL
BASE44_APP_URL = os.environ.get('BASE44_APP_URL', 'https://app-ffa38ee7.base44.app')
BASE44_FUNCTION_KEY = os.environ.get('BASE44_FUNCTION_KEY', '')
import json
...
BASE44_API_URL = os.environ.get('BASE44_APP_URL', 'https://app-ffa38ee7.base44.app')
...
def sync_user_to_base44(line_user_id, display_name, coach_tone, coach_style, quote_freq, total_messages):
    ...
    try:
        ...
        if resp.status_code == 200:
            print(f"[Base44 Sync] ✅ {display_name} 已同步")
        else:
            print(f"[Base44 Sync] ⚠️ {resp.status_code}")
    except Exception as e:
...
def save_goal_to_base44(line_user_id, display_name, title, goal_type='short', description='', target_date=''):
    ...
    try:
        ...
        if resp.status_code == 200:
            print(f"[Base44 Goal] ✅ 目標已儲存")
    except Exception as e:
        print(f"[Base44 Goal] 連線失敗: {e}")
...
def save_event_to_base44(line_user_id, display_name, title, event_type='todo', due_date='', recurrence='none'):
    ...
    try:
        ...
        if resp.status_code == 200:
            print(f"[Base44 Event] ✅ 事件已儲存")
    except Exception as e:
        print(f"[Base44 Event] 連線失敗: {e}")
...
def detect_goal_or_event(text):
    """簡單的關鍵字偵測，判斷是否提及目標或事件"""
    goal_keywords = ['目標', '想要', '達成', '設定', '計畫', '想做', '要完成']
    habit_keywords = ['習慣', '每天', '每週', '重複', '養成', '打卡', '堅持']
    todo_keywords = ['待辦', '做', '完成', '需要', '應該', '記得', '提醒我']
    
    is_goal = any(kw in text for kw in goal_keywords)
    is_habit = any(kw in text for kw in habit_keywords)
...
def detect_entity_intent(text):
    """簡單的關鍵字偵測，返回 (entity_type, is_goal_related)"""
    text_lower = text.lower()
    goal_keywords = ['目標', '目标', 'goal', '想要', '期望', '希望', '計劃', '计划', '達成', '達到']
    event_keywords = ['習慣', '习惯', 'habit', '待辦', '待办', 'todo', '完成', '做', '跑步', '運動', '运动']
...
def handle_message(event):
    ...
    def process_and_push():
        ...
        # 偵測是否要存目標/事件
        entity_type, _ = detect_entity_intent(user_text)
        if entity_type == 'goal':
            save_goal_to_base44(
                user_id,
