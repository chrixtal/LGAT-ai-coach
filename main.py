...
def save_goal_to_base44(line_user_id, display_name, title, description, type_):
    """儲存目標到 Base44"""
    try:
        resp = requests.post(
        ...
        if resp.status_code == 200:
            print(f"[saveGoal] 成功 | user={line_user_id}")
        else:
            print(f"[saveGoal] 失敗: {resp.text}")
    except Exception as e:
...
def handle_message(event):
    ...
    # 同步用戶資料到 Base44（不阻擋 webhook）
    new_profile = get_profile(user_id)
    new_profile['total_messages'] = (new_profile.get('total_messages', 0) or 0) + 1
    save_profile(user_id, total_messages=new_profile['total_messages'])
    
    threading.Thread(
        target=sync_user_to_base44,
        args=(user_id, new_profile),
        daemon=True
    ).start()
