...
def save_goal_or_event_to_base44(line_user_id, display_name, entity_type, **fields):
    ...
    try:
        ...
        resp = requests.post(url, json=payload, timeout=5)
        if resp.status_code == 200:
            print(f"[Base44] saveGoalOrEvent 成功: {entity_type}")
        else:
...
def detect_goal_or_event(text):
    """偵測用戶是否在設定目標或事件"""
    goal_keywords = ['目標', '計劃', '想要', '希望', '目的', '夢想', '要達成']
    event_keywords = ['待辦', '習慣', '打卡', '提醒', '安排', '計畫', '做']
    
    goal_matched = any(kw in text for kw in goal_keywords)
    event_matched = any(kw in text for kw in event_keywords)
    
    return goal_matched, event_matched
...
def handle_message(event):
    ...
    def process_and_push():
        ...
        if event_detected:
            title = user_text.split('。')[0][:80]
            save_goal_or_event_to_base44(user_id, display_name, 'event', title=title, type='todo')
